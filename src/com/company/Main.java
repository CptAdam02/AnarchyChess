...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        do{
            ...
            if(Moves.size() == 0){
                ...
                moveApplication(chessBoard, moveDecide(Moves));
            }
            Moves.clear();
    ...
    */
            ...
            for (int y = 0; y < chessBoard.length; y++){
                if (chessBoard[y][x].getColour() == playerTurn){
                    switch(chessBoard[y][x].getPieceType()){
                        case "pawn":
                            Moves.addAll(pawnCheck(chessBoard,playerTurn,y,x));
                            break;
                        case "rook":
                            Moves.addAll(rookCheck(chessBoard,playerTurn,y,x));
                            break;
                        case "knight":
                            Moves.addAll(knightCheck(chessBoard,playerTurn,y,x));
                            break;
                        case "bishop":
        ...
        for (int i= -1;i<2;i++){
            for ( int j =-1;j<2;j++){
                if ((i!=j) && (i!=-j)){
                   ...
                   Moves.addAll(directionCheck(chessBoard,playerTurn,x,y,j,i));
                }
            }
        }
    ...
    public static List<String> directionCheck(chessPiece[][] chessBoard,int playerTurn,int x,int y,int i,int j){
        List<String> Moves = new ArrayList<>();
        boolean cont = true;
